        ...
        for file in path.glob(f"{exp}*_prob.npy"):
            ...
            for ix in range(4):
                for iy in range(4):
                    ...
                    npad = ((int(z_diff), int(z_diff)), (0, 0), (0, 0))
                    tile_zcorr = np.roll(np.pad(mean_tiles[ix,iy], pad_width=npad, mode='constant', constant_values=0), shift=z_diff, axis=0)[z_diff:-z_diff]
